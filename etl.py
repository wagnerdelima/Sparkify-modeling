...
def process_song_file(cur, filepath) -> None:
    """
    Reads data from song source and inserts
    data into song and artist tables
    """
    ...
    params = ['artist_id', 'artist_name',
              ...
              'artist_longitude']
    artist_data = df[params].values[0]
    cur.execute(artist_table_insert, artist_data)


def process_log_file(cur, filepath) -> None:
    """
    Reads data from log source. Filter songs with NextSong
    and does a little casting: it casts the ts timestamp to ms;
    and process attributes of time. Saves data into user,
    time and songplay tables.
    """
    # open log file
    df = pd.read_json(filepath, lines=True)
...
def process_data(cur, conn, filepath, func) -> None:
    """
    Iterates over all datasource files and
    applies a function to datasource.
    """
...
def main():
    """
    Main method. Connects to database and handles data insertion.
    """
    connect = "host=127.0.0.1 dbname=sparkifydb user=student password=student"
